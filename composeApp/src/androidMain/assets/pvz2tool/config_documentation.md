...
- **行内图标 (Icon)**：
  - **格式**：`{{icon:文件名}}`。
  - **自动化处理**：组件会自动扫描文本中的 `icon` 标签，并尝试从 `${Pvz2ToolConfig.PATH_NAME}/images/文件名` 加载 Bitmap。
  - **视觉对齐**：图标大小会随字体大小（fontSize）自动缩放（约 1.2 倍），并保持垂直居中。
  - 示例：`消耗 {{icon:sun.png}} 50 点阳光`。

---

### JS 脚本内置对象

`BUTTON` 类型栏目的 `jsScript` 脚本运行在 QuickJS 沙箱环境中，可使用以下全局对象。
