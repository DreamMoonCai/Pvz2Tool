...
- **交互式链接 (Link)**：
  - **本质**：链接也是一种「点击触发」的交互——地址可以是普通网址（点击用系统浏览器打开），也可以是 JS 代码或 `.js` 文件（点击即执行，相当于点击式的 `{{js:...}}`）。
  - **基础格式**：`{{link|URL:显示文本}}`。
...
- **行内图标 (Icon)**：
  - **格式**：`{{icon|width=宽度|height=高度|x=偏移|y=偏移|z=层级:文件名}}`（所有参数均可选，单位 dp）。
  - **参数**：
     - `文件名`：`pvz2tool/images/` 下的图片文件名；以 `/` 开头则为绝对路径（如 `/data/data/com.example/files/icon.png`）。
     - `width=宽度`（可选）：图片宽度，单位 dp。**省略时不生成该字段，由解析器按 `fontSize * 1.2` 自适应**。
     - `height=高度`（可选）：图片高度，单位 dp；省略时同样按 `fontSize * 1.2` 自适应。
     - `x=偏移` / `y=偏移`（可选）：单位 dp。**x 或 y 任一存在即进入「浮层模式」**：
       - 图标**不占**原文字空间，而是按「整段文本起点（首个文字处，换行不重算）+ (x,y)dp」浮在文字之上或之下。
       - `x` 向右为正、`y` 向下为正（负值向左/上）。
       - **需要周围有文字提供起点锚点**；若整段只有图标本身而无其他文字，浮层模式可能因无文本起点而定位失败、不显示。
     - `z=层级`（可选，仅浮层模式有效）：控制图标与各文字/图标间的层叠顺序。`z>0` 浮于文字之上，`z<0` 沉于文字之下，不同图标可用不同 z 分层。
  - **自动化处理**：组件会自动扫描文本中的 `icon` 标签，并尝试从 `${Pvz2ToolConfig.PATH_NAME}/images/文件名` 加载图片（绝对路径则直接从本地文件系统加载）。
  - **视觉对齐**：普通（非浮层）图标大小会随字体大小（fontSize）自动缩放（约 1.2 倍），并保持垂直居中。
  - **示例**：
     - `消耗 {{icon:sun.png}} 50 点阳光`（默认宽度/高度，随字号自适应）。
     - `{{icon|width=32|height=32:coin.png}}`（32dp 正方形图标）。
     - `{{icon|width=24|height=24:/data/data/com.example/files/sword.png}}`（绝对路径图标）。
     - `示例文字 {{icon|x=10|y=-6|z=2:auto_collect.png}} 示例文字`（浮层图标：从文本起点右移 10dp、上移 6dp、压在文字之上，且不占文字空间）。
